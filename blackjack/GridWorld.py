        ...
        for i in range(WORLD_SIZE):
            for j in range(WORLD_SIZE):
                ...
                for action in actions:
                    next_state = nextState[i][j][action]
                    next_values.append(actionReward[i][j][action] + discount * world[next_state[0]][next_state[1]])
                #next_world[i][j] = max(next_values)
                new_value = max(next_values)
                difference = max(new_value - world[i][j], difference)
                world[i][j] = new_value
        #world=next_world
        ## End your code
		# keep iteration until convergence
    ...
    while True:
        ...
        for i in range(WORLD_SIZE):
            for j in range(WORLD_SIZE):
                ...
                next_state = nextState[i][j][action]
                #next_world[i][j] = actionReward[i][j][action] + discount * world[next_state[0]][next_state[1]]
                new_value = actionReward[i][j][action] + discount * world[next_state[0]][next_state[1]]
                #difference = max(next_world[i][j] - world[i][j], difference)
                difference = max(new_value - world[i][j], difference)
                world[i][j]=new_value
        #world=next_world
        ## End your code
...
def policy_iteration(nextState, actionReward):
    ...
    while True:
        ...
        for i in range(WORLD_SIZE):
            for j in range(WORLD_SIZE):
                next_state=nextState[i][j][actions[policy[i][j]]]
                for action in actions:
                    new_next_state = nextState[i][j][action]
                    if world[new_next_state[0]][new_next_state[1]]>world[next_state[0]][next_state[1]]:
